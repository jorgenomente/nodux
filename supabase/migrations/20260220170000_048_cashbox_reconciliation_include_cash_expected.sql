-- Cashbox reconciliation rows: include expected cash total row + keep device rows and MercadoPago aggregated row.
...
begin
  ...
  grouped as (
    select ec.*
    from expected_cash_row ec
    union all
    select gnm.*
    from grouped_non_mp gnm
    union all
    select gm.*
    from grouped_mp gm
