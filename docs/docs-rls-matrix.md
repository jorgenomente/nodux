...
- Gate org-wide para enqueue fiscal productivo agregado en `supabase/migrations/20260309102000_082_fiscal_prod_enqueue_gate.sql` (`org_preferences.fiscal_prod_enqueue_enabled`; sin cambios de policy, reusa RLS existente de `org_preferences` y endurece `rpc_enqueue_sale_fiscal_invoice` en ambiente `prod`).
