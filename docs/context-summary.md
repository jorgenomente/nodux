...
- Se aprueba el diseño del próximo track comercial SaaS: `/superadmin/subscriptions` para control de suscripciones por org y `/settings/membership` para visibilidad de plan/pagos manuales por organización. El modelo recomendado usa plan base con 1 sucursal incluida, adicional por sucursal activa extra, pricing `standard|custom`, comprobantes manuales y revisión SA.
- UI inicial de suscripciones SaaS implementada: `/superadmin/subscriptions` permite pricing, estados de servicio, revisión manual de pagos, activación/desactivación de org/sucursal y edición de datos bancarios/QR/instrucciones. `/settings/membership` permite a OA ver plan/monto mensual y subir comprobantes al bucket privado `subscription-payment-proofs`.
- Iteración de UX/comercial aplicada sobre suscripciones: el QR de cobro ya se sube realmente desde superadmin a `platform-billing-assets` y se muestra en `/settings/membership` mediante signed URL server-side. `/superadmin/subscriptions` además agrega CTA para activar una org e ingresar directo a `/dashboard`.
- Segunda iteración UX aplicada sobre suscripciones: `/superadmin/subscriptions` ya filtra por servicio/pago/pricing, muestra KPIs comerciales de cartera y resalta orgs en gracia, suspendidas o con comprobantes pendientes/rechazados. `/settings/membership` ahora muestra banners claros de gracia, suspensión, deuda vencida, comprobante enviado y rechazo, más un bloque previo con monto esperado y vencimiento para informar pagos.
- El modelo comercial de suscripciones ahora soporta bonificaciones visibles por `%` o monto fijo. Superadmin configura `precio lista + bonificación + total final` y la org ve explícitamente cuánto ahorra por mes en `/settings/membership`.
- UX de `Configuración comercial` en `/superadmin/subscriptions` refactorizada: ya no es un formulario largo plano. Ahora se presenta como configurador comercial guiado con bloques separados para `Lo que ve el cliente`, `Cómo se calcula`, `Bonificación`, `Vigencia y ciclo` y `Notas`, con campos condicionales según `standard/custom` y tipo de bonificación.
